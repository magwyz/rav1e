  ...
  if !must_split {
    ...
    if !can_split {
      if !mode_decision.pred_mode_luma.is_intra() {
        ...
        save_block_motion(
          fi, fs, mode_decision.bsize, &mode_decision.bo, mode_decision.mvs[0]
        );
      }
    ...
    if best_partition != PartitionType::PARTITION_SPLIT {
        ...
        for mode in rdo_output.part_modes.clone() {
          ...
          if !mode.pred_mode_luma.is_intra() {
            // Fill the saved motion structure
            save_block_motion(
              fi, fs, mode.bsize, &mode.bo, mode.mvs[0]
            );
          }
  ...
  match partition {
    PartitionType::PARTITION_NONE => {
      ...
      if !mode_luma.is_intra() {
        ...
        save_block_motion(fi, fs, part_decision.bsize, &part_decision.bo, mvs[0]);
      }
