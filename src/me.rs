...
fn get_best_predictor(fi: &FrameInvariants,
  ...
  center_mv: &mut MotionVector, center_mv_cost: &mut u32,
  tmp_plane_opt: &mut Option<Plane>, ref_frame: usize) {
  *center_mv = MotionVector{row: 0, col: 0};
  *center_mv_cost = std::u32::MAX;
...
fn get_mv_rd_cost(
  ...
  mvx_min: isize, mvx_max: isize, mvy_min: isize, mvy_max: isize,
  blk_w: usize, blk_h: usize,
  cand_mv: MotionVector, tmp_plane_opt: &mut Option<Plane>,
  ref_frame: usize) -> u32
{
...
fn compute_mv_rd_cost(
  fi: &FrameInvariants,
  pmv: &[MotionVector; 2], lambda: u32,
  bit_depth: usize, blk_w: usize, blk_h: usize, cand_mv: MotionVector,
  plane_org: &PlaneSlice, plane_ref: &PlaneSlice
) -> u32
{
...
fn telescopic_subpel_search(
  fi: &FrameInvariants, fs: &FrameState, bsize: BlockSize, po: &PlaneOffset,
  lambda: u32, ref_frame: usize, pmv: &[MotionVector; 2],
  mvx_min: isize, mvx_max: isize, mvy_min: isize, mvy_max: isize,
  tmp_plane: &mut Plane, best_mv: &mut MotionVector, lowest_cost: &mut u32
) {
  let blk_w = bsize.width();
  let blk_h = bsize.height();
